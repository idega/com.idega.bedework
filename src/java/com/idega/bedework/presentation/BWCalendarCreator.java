 ...
 */
public class BWCalendarCreator extends Block {
	...
	public void main(IWContext iwc) throws Exception {
		...
		PresentationUtil.addStyleSheetsToHeader(iwc, Arrays.asList(
				bundle.getVirtualPathWithFileNameString(
						FILE_CALENDAR_CREATOR_CSS
						)
		));

		if (!iwc.isLoggedOn()) {
			add(new Heading3(iwrb.getLocalizedString("user_not_logged_on", "User is not logged on")));
			return;
		}
		...
		if (PARAMETER_CREATE.equals(action)) {
			...
			/* Prints results. */
			if (isUpdated) {
				add(new Heading3(iwrb.getLocalizedString("your_calendar_is_updated",
						"Your calendar is updated!")));
			} else {
				add(new Heading3(iwrb.getLocalizedString("failed_to_updated",
						"Failed to update. Check your form!")));
			}
		...
		/* User asked to remove calendar. */
			...
			/* Prints results. */
			if (isRemoved) {
				add(new Heading3(iwrb.getLocalizedString("calendar_successfully_removed",
						"Calendar removed")));
			} else {
				add(new Heading3(iwrb.getLocalizedString("failed_to_remove_calendar",
		...
		/* User asked to open edit form. */
			...
			if (calendar == null) {
				add(new Heading3(iwrb.getLocalizedString("not_possible_to_edit",
						"Not possible to edit default calendar")));
			} else {
		...
		/* User asked nothing, opening calendar view. */
			...
			if (addCalendarsViewHeaders(iwc, table.createHeaderRowGroup())
				...
				add(new Heading3(iwrb.getLocalizedString(
						"no_calendars_found",
						"No calendars found."))
				);
			}
	...
	private Layer getGroupsSelectionBox(IWContext iwc, Set<Long> groupIDs) {
		...
		groupsSelectionBoxLayer.add(new Text(
				iwrb.getLocalizedString("visible_for_groups", "Visible for groups:")));
	...
	private boolean addCalendarsViewHeaders(IWContext iwc,
		...
		cell.add(new Text(iwrb.getLocalizedString("number", "Nr.")));
		...
		cell = header.createCell();
		cell.setStyleClass(CLASS_CALENDARS_TABLE_CELL);
		cell.add(new Text(iwrb.getLocalizedString("delete", "Delete")));
	...
	private boolean addCalendarViewRow(IWContext iwc, BwCalendar calendar,
		...
		Link edit = new Link(bundle.getImage(FILE_CALENDAR_IMAGE_EDIT,
				iwrb.getLocalizedString("edit", "Edit")));
		...
		Link delete = new Link(bundle.getImage(FILE_CALENDAR_IMAGE_DELETE,
				iwrb.getLocalizedString("remove", "Remove"))	);
	...
	private Layer getNameLayer(IWContext iwc, String name) {
		...
		Label inputLabel = new Label(iwrb
	...
	private Layer getPublicPrivateSelection(IWContext iwc, Boolean isPublic) {
		...
		radioButtonGroup.addRadioButton(publicButton,
				new Text(iwrb.getLocalizedString(VALUE_CALENDAR_PUBLIC, "No")));
		...
		radioButtonGroup.addRadioButton(privateButton,
				new Text(iwrb.getLocalizedString(VALUE_CALENDAR_PRIVATE, "Yes")),
				Boolean.TRUE);
	...
	private Layer getButtonsForCalendarAdditionLayer(IWContext iwc) {
		...
		BackButton backButton = new BackButton(
				iwrb.getLocalizedString("back", "Back"));
