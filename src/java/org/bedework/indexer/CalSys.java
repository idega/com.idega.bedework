...
public abstract class CalSys {
   ...
   * @param principal
   */
  public void setCurrentPrincipal(final boolean publick,
  ...
  public CalSvcI getAdminSvci() throws CalFacadeException {
    CalSvcIPars pars = CalSvcIPars.getServicePars(adminAccount,
                                                  true,   // publicAdmin,
                                                  true);   // Allow super user
    CalSvcI svci = new CalSvcFactoryIdega().getSvc(pars);
  ...
  private String showPrincipal() {
    if (principal != null) {
      return "publick=" + publick + " principal=" + principal;
    }

    return "publick=" + publick + " account=" + adminAccount;
  }
